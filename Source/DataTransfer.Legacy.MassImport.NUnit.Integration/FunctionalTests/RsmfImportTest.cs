...
{
	[TestFixture]
	public class RsmfImportTest : MassImportTestBase
	{
		...
		[Test]
		public async Task ShouldRunRsmfImport()
		{
			// Arrange
		...
		private async Task<NativeLoadInfo> CreateSampleNativeLoadInfoAsync(int numberOfArtifactsToCreate)
		{
			...
			string dataFileContent = GetMetadata(_fieldDelimiter, _expectedFieldValues, _expectedFolders, includeMetadataFileId: true);
