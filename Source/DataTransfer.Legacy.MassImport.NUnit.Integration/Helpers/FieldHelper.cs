...
namespace MassImport.NUnit.Integration.Helpers
{
	...
	{
		private const string _extractedTextFieldName = "Extracted Text";
		...
		public static async Task<MassImportField> ReadExtractedTextField(IntegrationTestParameters parameters, TestWorkspace workspace)
		{
			var fieldsToRead = new[] { "Name", "Unicode", "EnableDataGrid" };
			QueryRequest queryRequest = new QueryRequest
			{
				ObjectType = new ObjectTypeRef
				{
				...
				Condition = $"'Object Type Artifact Type ID' IN ['{(int)ArtifactType.Document}'] AND 'Name' == '{_extractedTextFieldName}'",
				Fields = fieldsToRead.Select(fieldName => new FieldRef { Name = fieldName }).ToArray(),
			};
			...
			{
				ArtifactID = fieldFields.ArtifactID,
				Category = Relativity.FieldCategory.FullText,
				Type = FieldTypeHelper.FieldType.Text,
				DisplayName = (string)fieldFields["Name"].Value,
				IsUnicodeEnabled = (bool)fieldFields["Unicode"].Value,
				EnableDataGrid = (bool)fieldFields["EnableDataGrid"].Value,
			};
		}
		...
		public static async Task EnableDataGridForFieldAsync(IntegrationTestParameters parameters, TestWorkspace workspace, int longTextFieldId, string longTextName)
		{
			using (IWorkspaceManager workspaceManager = ServiceHelper.GetServiceProxy<IWorkspaceManager>(parameters))
			{
				var workspaceResponse = await workspaceManager.ReadAsync(workspace.WorkspaceId).ConfigureAwait(false);

				if (!workspaceResponse.EnableDataGrid)
				{
					var updateWorkspaceRequest = new WorkspaceRequest(workspaceResponse)
					{
						EnableDataGrid = true
					};

					await workspaceManager.UpdateAsync(workspace.WorkspaceId, updateWorkspaceRequest).ConfigureAwait(false);
				}
			}
			...
			using (IFieldManager fieldManager = ServiceHelper.GetServiceProxy<IFieldManager>(parameters))
			{
				...
				await fieldManager.UpdateLongTextFieldAsync(workspace.WorkspaceId, longTextFieldId, longTextFieldRequest).ConfigureAwait(false);
			}
		}
	}
}