	...
	internal class NativeImportCommand : ImportCommandBase
	{
		...
		protected void WriteDataGridInformation()
		{
			if (DataGridInputReaderProvider.IsDataGridInputValid())
			{
				...
				{
					this.Execute(() => Native.UpdateDgFieldMappingRecords(Input.DGImportFileInfo.ToArray(), this.CorrelationLogger, Context.AppArtifactID), nameof(WriteDataGridInformation));
				}
			}
		}
	}
