	...
	internal abstract class ObjectBase : IObjectBase, IDataGridInputReaderProvider
	{
			...
			bool useLegacyDG = true)
		{
			...
			if (settings.HasDataGridWorkToDo)
			{
				...
				// 1. CAL usage disabled with a toggle, 2. Other stages than import to DG, 3. ExtractedText is imported as a value, 4. ExtractedText is not imported, 5. ExtractedText is imported to SQL
				if (ToggleProvider.Current.IsEnabled<DisableCALForDataGridToggle>() || useLegacyDG || !Settings.LoadImportedFullTextFromServer || _fullTextField == null || !_fullTextField.EnableDataGrid)
				{
