...
namespace Relativity.DataTransfer.Legacy.Services.Installer
{
	public class ServicesInstaller : IWindsorInstaller
	{
		public void Install(IWindsorContainer container, IConfigurationStore store)
		{
			...
			container.Register(Component.For<IAPILog>().UsingFactoryMethod(k => { return new RelEyeLogger(TelemetryConstants.Values.ServiceName, k.Resolve<IHelper>().GetLoggerFactory().GetLogger(), k.Resolve<IHelper>().GetInstanceSettingBundle()); }).LifestyleSingleton());
			...
			container.Register(Component.For<IServiceHelper>().UsingFactoryMethod(() => (IServiceHelper)container.Resolve<IHelper>()).LifestyleSingleton());
			container.Register(Component.For<IAuthenticationMgr>()
				.UsingFactoryMethod(x => x.Resolve<IServiceHelper>().GetAuthenticationManager(), managedExternally: true).LifestyleTransient());
			...
			container.Register(Component.For<IFileRepositoryExternalService>().ImplementedBy<KeplerFileRepositoryExternalService>().LifestyleTransient());
